...
export interface EspnRankingSource extends Omit<RankingSource,'updatedAt'|'rankings'> { id:'ESPN'; label:'ESPN PPR300'; season:number; sourceUpdatedAt:string|null; importedAt:string; originalFilename:string; detectedRows:number; diagnostics:EspnImportDiagnostics; rankings:Map<CanonicalPlayerId,EspnRankingValue> }
...
export interface EspnImportPreview { metadata:EspnImportMetadata; rows:EspnImportRow[]; matched:{row:EspnImportRow;player:DraftPlayer;manual?:boolean}[]; unmatched:EspnImportRow[]; ambiguous:{row:EspnImportRow;candidates:DraftPlayer[]}[]; invalid:{row:number;reason:string}[]; ignored:EspnImportRow[] }
...
export function reconcileEspnRows(rows:EspnImportRow[],players:DraftPlayer[],metadata:EspnImportMetadata,invalid:EspnImportPreview['invalid']=[]):EspnImportPreview{
  ...
  return{metadata,rows,matched,unmatched,ambiguous,invalid,ignored:[]};
}
...
const unresolvedEspnRow=(preview:EspnImportPreview,rank:number)=>preview.unmatched.find(item=>item.overall_rank===rank)??preview.ambiguous.find(item=>item.row.overall_rank===rank)?.row;
export function manuallyAssignEspnRow(preview:EspnImportPreview,rank:number,player:DraftPlayer):EspnImportPreview{const row=unresolvedEspnRow(preview,rank);if(!row||!player.canonicalPlayerId)throw new Error('Manual assignment requires an unresolved row and a canonical player.');return{...preview,matched:[...preview.matched,{row,player,manual:true}],unmatched:preview.unmatched.filter(item=>item!==row),ambiguous:preview.ambiguous.filter(item=>item.row!==row)}}
export function ignoreEspnRow(preview:EspnImportPreview,rank:number):EspnImportPreview{const row=unresolvedEspnRow(preview,rank);if(!row)throw new Error('Only an unresolved ESPN row can be ignored.');return{...preview,ignored:[...preview.ignored,row],unmatched:preview.unmatched.filter(item=>item!==row),ambiguous:preview.ambiguous.filter(item=>item.row!==row)}}
export const espnImportDiagnostics=(preview:EspnImportPreview):EspnImportDiagnostics=>({matched:preview.matched.length,unmatched:preview.unmatched.length,ambiguous:preview.ambiguous.length,invalid:preview.invalid.length,ignored:preview.ignored.length});
