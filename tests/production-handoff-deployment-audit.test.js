...
for(const marker of ['information_schema.columns','pg_get_functiondef','pg_try_advisory_xact_lock','pg_advisory_xact_lock','lock_timeout','55P03','supabase_migrations.schema_migrations','pg_indexes','pg_constraint','pg_trigger']){
  assert.ok(verification.includes(marker), `verification includes ${marker}`);
}
for(const commit of ['be5116c','b0b84ed','f17529c','9b7c7b3','843fc3e','5522ceb']) assert.ok(audit.includes(commit), `audit inventories ${commit}`);
