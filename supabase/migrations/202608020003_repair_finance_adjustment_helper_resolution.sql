-- Make the Finance correction dependency deterministic in live deployments.
-- The helper is declared before the RPC and every helper key is explicitly text.
begin;

create or replace function public.finance_adjustment_value(
  p_adjustments jsonb,
  p_key text
) returns numeric
language plpgsql
...
declare
  ...
  v_allowed_keys constant text[] := array[
    'amount','original_amount','sales_tax_rate','sales_tax_collected',
    'shipping_charged','shipping_cost','material_cost','packaging_cost',
    'labor_cost','other_direct_cost','business_use_percent','miles_driven',
    'mileage_rate'
  ];
begin
  if p_key is null or not (p_key = any(v_allowed_keys)) then
    raise exception 'Unknown Finance adjustment key: %', coalesce(p_key, '<null>')
      using errcode = '22023';
  end if;
  if not (p_adjustments ? p_key) then return 0; end if;
  if jsonb_typeof(p_adjustments -> p_key) <> 'number' then
    raise exception 'Finance adjustment % must be a JSON number', p_key
      using errcode = '22023';
  end if;
  v_value := (p_adjustments ->> p_key)::numeric;
  if v_value::text in ('NaN','Infinity','-Infinity') or abs(v_value) > 999999999.99 then
    raise exception 'Finance adjustment % must be finite and within range', p_key
      using errcode = '22003';
  end if;
...
revoke all on function public.finance_adjustment_value(jsonb,text)
...
revoke all on function public.correct_financial_entry(uuid,jsonb,text[],text,timestamptz,boolean,text,text)
  from public, anon;
grant execute on function public.correct_financial_entry(uuid,jsonb,text[],text,timestamptz,boolean,text,text)
  to authenticated, service_role;

comment on function public.finance_adjustment_value(jsonb,text) is
  'Validates and rounds a canonical numeric Finance JSON field. Unknown keys reject with SQLSTATE 22023.';
comment on function public.correct_financial_entry(uuid,jsonb,text[],text,timestamptz,boolean,text,text) is
  'Append-only Finance correction with an explicit jsonb,text adjustment-helper dependency.';
