...
## Phase 5 implementation note — Deterministic intelligence

Phase 5 derives recommendations in memory from the authoritative league setup, draft plan/state, static player baseline, and `DraftUserContext`. The pure recommendation service never persists derived output and makes no network request. Player fixture records expose an honest `baselineRank`, `baselineValue`, and `fixtureTier` contract so Phase 6 can replace the source without replacing the recommendation model.

Scores use centralized, comparable component ranges: baseline value (0–10), roster fit (0–4), scarcity (0–4), tier pressure (0–3), Cost of Waiting (0–4), and next-turn risk (0–2). Philosophy (±1.25), interest (normally ±1), intent (±0.75), and bye overlap (at most −0.35) remain separately visible bounded context. Explicit HARD wait rules suppress a player unless objective scarcity/wait pressure reaches the override window; AVOID is a strong but transparent penalty.
