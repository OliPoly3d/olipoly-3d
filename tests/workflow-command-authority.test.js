...
assert(production.includes('const authoritative = await syncProductionStatusToOrder(j, status, j, commandContext)'), 'success must consume server-returned row with the unchanged action identity');
...
assert(/const recovery = pendingLinkedWorkflowRecovery\(j, status\)[\s\S]*?if\(!recovery\)\{[\s\S]*?consumeCapturedAttempt\(updated[\s\S]*?rememberLinkedWorkflowRecovery\(j, status, updated, lifecycleAction\)[\s\S]*?syncProductionStatusToOrder\(j, status, j, commandContext\)[\s\S]*?clearLinkedWorkflowRecovery\(id\)/.test(production), 'RPC failure followed by explicit operator retry must reuse pending Inventory work and clear recovery only after command success');
