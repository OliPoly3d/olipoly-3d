...
export function bootstrapPrimaryIdentities(sources:ParsedSource[],existing:DraftPlayer[]):DraftPlayer[]{
 ...
 for(const rows of evidence.values()){const sourceIds=new Set(rows.map(row=>row.source)),teams=new Set(rows.map(row=>normalizeTeam(row.team)??'FA')),sample=rows[0],name=normalizePlayerName(sample.playerName),position=family(sample.position) as Position;if((sourceIds.size<2&&position!=='DST')||teams.size!==1)continue;
