...
function textMoneyToNumber(text) { return Number(String(text || '').replace(/[^0-9.-]/g, '')) || 0; }
function quoteShippingModeToFulfillment(mode) { if (mode === 'pickup') return 'pickup'; if (mode === 'delivery') return 'delivery'; return 'shipping'; }
function padQuoteNumber(n) { return String(n).padStart(6, '0'); }
function formatQuoteNumber(n) { return `Q-${padQuoteNumber(n)}`; }
function formatInvoiceNumber(n) { return `INV-${padQuoteNumber(n)}`; }
const readHistory = () => { try { return JSON.parse(localStorage.getItem(STORAGE_KEY) || '[]'); } catch { return []; } };
const writeHistory = list => { localStorage.setItem(STORAGE_KEY, JSON.stringify(list)); refreshHistoryUI(); };
function nextLocalFallbackNumber() {
  const list = readHistory();
  const nums = list.map(q => Number(String(q.quoteNumber || '').replace(/[^\d]/g, ''))).filter(n => Number.isFinite(n) && n > 0);
...
function clearMissingHighlights() { document.querySelectorAll('.field-missing').forEach(el => el.classList.remove('field-missing')); }
...
function applyMissingHighlights(issues) {
  ...
  issues.forEach(issue => (map[issue] || []).forEach(id => els[id] && els[id].classList.add('field-missing')));
}
...
function alertMissingInputsIfNeeded(issues) { if (issues.length) alert(`Quote still has missing inputs:\n\n- ${issues.join('\n- ')}`); }
function buildAcceptedQuoteOrderPayload(userId) {
  const total = textMoneyToNumber(els.outFinal.textContent), deposit = textMoneyToNumber(els.outDeposit.textContent), balance = textMoneyToNumber(els.outBalance.textContent), depositRequired = deposit > 0;
  return {
    user_id: userId, order_number: els.quoteNumber.value.trim(), order_date: els.quoteDate.value || today(), customer_name: (els.companyName.value || els.customerName.value).trim() || null,
    customer_email: els.customerEmail.value.trim() || null, order_title: els.quoteTitle.value.trim() || 'Quoted Project', quantity: Math.max(1, num(els.qty.value) || 1), order_total: total,
    deposit_amount: deposit, balance_amount: balance, status: depositRequired ? 'awaiting_deposit' : 'in_design', payment_status: depositRequired ? 'deposit_due' : 'unpaid',
    fulfillment: quoteShippingModeToFulfillment(els.shippingMode.value || 'pickup'), tracking_number: null, payment_link: null,
    internal_notes: [`Created automatically from accepted quote ${els.quoteNumber.value.trim() || ''}`.trim(), els.quoteNotes.value.trim(), els.assumptions.value.trim(), els.poNumber.value.trim() ? `PO #: ${els.poNumber.value.trim()}` : '', els.invoiceNumber.value.trim() ? `Invoice #: ${els.invoiceNumber.value.trim()}` : ''].filter(Boolean).join('\n\n'),
    ...
    shipping_or_pickup_note: els.shippingMode.value === 'pickup' ? 'Pickup details will be shared as your order progresses.' : els.shippingMode.value === 'delivery' ? 'Delivery details will be shared as your order progresses.' : 'Shipping details and tracking will be added once available.'
  };
}
function buildAcceptedQuotePublicTrackingPayload(orderPayload) {
  return { order_number: orderPayload.order_number, user_id: orderPayload.user_id, order_title: orderPayload.order_title || null, status: orderPayload.status, payment_status: orderPayload.payment_status, public_status_text: orderPayload.public_status_text || null, public_next_step: orderPayload.public_next_step || null, shipping_or_pickup_note: orderPayload.shipping_or_pickup_note || null, tracking_number: orderPayload.tracking_number || null, payment_link: orderPayload.payment_link || null };
}
async function syncAcceptedQuoteToOrders() {
  if (els.quoteStatus.value !== 'accepted') return;
  const quoteNumber = els.quoteNumber.value.trim(), quoteTitle = els.quoteTitle.value.trim();
  ...
  const orderRes = await sbApi('/rest/v1/orders?on_conflict=order_number', { method: 'POST', headers: { Prefer: 'resolution=merge-duplicates,return=representation' }, body: JSON.stringify(orderPayload) });
  ...
  const publicRes = await sbApi('/rest/v1/order_tracking_public?on_conflict=order_number', { method: 'POST', headers: { Prefer: 'resolution=merge-duplicates,return=representation' }, body: JSON.stringify(buildAcceptedQuotePublicTrackingPayload(orderPayload)) });
...
function defaultAssumptions() {
  const t = els.orderType.value || 'custom', ship = els.shippingMode.value || 'pickup';
  const shippingLine = ship === 'pickup' ? 'Local pickup is assumed unless otherwise noted.' : ship === 'delivery' ? 'Local delivery pricing is estimated from the current delivery assumption and may adjust if destination changes.' : ship === 'ship_customer_label' ? 'Customer-provided shipping label is assumed and outbound shipping cost is not included in this quote.' : 'Shipping, if applicable, is estimated and may adjust if final packaging size, weight, or destination changes.';
...
function paymentTermsText() {
  const map = { due_on_receipt: 'Payment due on receipt of invoice.', deposit_to_start: 'Deposit is required before production begins. Remaining balance is due before delivery or pickup unless otherwise arranged.', due_on_completion: 'Payment is due upon project completion, before pickup, delivery, or shipment.', net_15: 'Payment is due within 15 days of invoice date.', net_30: 'Payment is due within 30 days of invoice date.' };
...
function applyShippingMode() {
  ...
  if (mode === 'pickup') { if (!num(els.simpleShipping.value)) els.simpleShipping.value = 0; }
  else if (mode === 'ship_customer_label') els.simpleShipping.value = 0;
}
function applyProfessionalMode() {
  ...
  els.modeHint.textContent = on ? 'Professional / Bulk Client Mode is on. Quote PDFs use more formal company wording, and invoice PDFs are available for deposit, final, or full billing.' : 'Standard mode is best for regular customer quotes. Turn on Professional / Bulk Client Mode when you want a more formal quote PDF and company-style invoice.';
  if (on) {
...
function applyOrderType() {
  const t = els.orderType.value || 'custom', isCraft = t === 'craft_show', isBulk = t === 'business_bulk', isRepeat = t === 'repeat';
  els.customerName.placeholder = isBulk ? 'Buyer / requestor name' : 'Optional';
  if (isBulk) els.professionalMode.value = 'on';
  if (isCraft) els.depositPercent.value = 0; else if (isRepeat) els.depositPercent.value = 25; else if (isBulk) els.depositPercent.value = 50; else if (!num(els.depositPercent.value)) els.depositPercent.value = 50;
  ...
  if (!els.customerNotes.value.trim()) {
    els.customerNotes.value = isCraft ? 'Pre-made inventory item prepared for sale. Pricing reflects finished stock on hand unless otherwise noted.' : isBulk ? 'Quote prepared for a business / multi-unit order. Final schedule and delivery details can be confirmed at approval.' : isRepeat ? 'Quote prepared from a prior or repeat-style order with current materials and labor assumptions.' : 'Custom 3D printed item quoted from the specifications discussed.';
  }
  ...
  applyProfessionalMode(); applyShippingMode();
}
...
function applySimpleInputs() {
  const labels = [...els.directItems.children], overhead = [...els.overheadItems.children], mat = simpleMaterialTotal(), colorCount = Math.max(1, Math.min(4, num(els.filamentCount.value) || 1));
...
function suggestedProfitPercent() {
  ...
  const aggressive = els.suggestedMode.value === 'aggressive', preset = els.presetSelect.value;
  let pct = 35;
...
function financeEntryText(finalQuote, beforeTax, tax) {
  ...
  const byName = key => { const row = rows.find(r => (r.querySelector('.item-label').value || '').toLowerCase().includes(key)); return row ? rowValue(row) : 0; };
  return ['Type: Income','Category: Sale',`Title: ${els.quoteTitle.value.trim() || 'Accepted Quote'}`,`Base Amount: ${beforeTax.toFixed(2)}`,'Shipping Charged: 0.00','Amount Includes Sales Tax?: No',`Sales Tax Collected: ${tax.toFixed(2)}`,`Material Cost: ${byName('filament').toFixed(2)}`,`Packaging Cost: ${byName('packaging').toFixed(2)}`,'Labor Cost: 0.00',`Other Direct Cost: ${(totalOf(els.overheadItems) + byName('hardware')).toFixed(2)}`,`Shipping Cost You Paid: ${byName('shipping').toFixed(2)}`,`Customer Total Paid: ${finalQuote.toFixed(2)}`,`Notes: Customer: ${els.customerName.value || els.companyName.value || 'N/A'} | Quote #: ${els.quoteNumber.value || 'N/A'}${els.invoiceNumber.value ? ` | Invoice #: ${els.invoiceNumber.value}` : ''}`].join('\n');
}
function invoiceAmount(total, deposit, balance) { if (els.invoiceType.value === 'deposit') return deposit; if (els.invoiceType.value === 'final') return balance; return total; }
function documentClientName() { return els.professionalMode.value === 'on' ? (els.companyName.value.trim() || els.customerName.value.trim() || '—') : (els.customerName.value.trim() || els.companyName.value.trim() || '—'); }
function fillPdf(mode, total, beforeTax, tax, deposit, balance, perItem) {
  const isInvoice = mode === 'invoice', professional = els.professionalMode.value === 'on';
  if (els.pdfSheet) {
  ...
  els.pdfSubtitle.textContent = isInvoice ? (els.invoiceType.value === 'deposit' ? 'Deposit request for approved work.' : els.invoiceType.value === 'final' ? 'Final balance invoice for completed or approved work.' : 'Full invoice for the quoted order.') : (professional ? 'Formal quote prepared for review and approval.' : (els.customerNotes.value.trim() || 'A polished customer-ready quote for your order.'));
  ...
  els.pdfCustomerNotes.textContent = isInvoice ? (els.invoiceNotes.value.trim() || 'Please remit payment according to the terms listed below.') : (els.customerNotes.value.trim() || 'No customer-facing notes added yet.');
  els.pdfAssumptions.textContent = isInvoice ? (professional ? `Project reference: ${els.quoteTitle.value.trim() || 'Quoted project'}${els.poNumber.value.trim() ? ` | PO #: ${els.poNumber.value.trim()}` : ''}` : `Project reference: ${els.quoteTitle.value.trim() || 'Quoted project'}`) : (els.assumptions.value.trim() || 'No exclusions or assumptions added yet.');
  els.pdfInvoiceTerms.innerHTML = `<strong>Payment Terms</strong><br>${paymentTermsText()}` + `${els.customerEmail.value.trim() ? `<br>Billing contact: ${els.customerEmail.value.trim()}` : ''}` + `${els.invoiceNotes.value.trim() ? `<br><br>${els.invoiceNotes.value.trim()}` : ''}`;
...
function render() {
  ...
  const materialTotal = simpleMaterialTotal(), machineTotal = num(els.machineHours.value) * num(els.machineRate.value), designTotal = num(els.designHours.value) * num(els.designRate.value), postTotal = num(els.postHours.value) * num(els.postRate.value);
  ...
  const pairs = { sumDirect: direct, sumOverhead: overhead, sumProfit: actualProfit, sumDeposit: deposit, sumBalance: balance, sumQuote: finalQuote, sumPerItem: perItem, sumBreakEven: base, outDirect: direct, outOverhead: overhead, outBase: base, outProfit: actualProfit, outPerItem: perItem, outBreakEven: base, outPreDiscount: preDiscount, outBeforeTax: beforeTaxRaw, outRoundedBeforeTax: beforeTax, outRoundingGain: roundingGain, outTax: tax, outDeposit: deposit, outBalance: balance, outFinal: finalQuote };
  ...
  let confidenceText = 'Ready', confidenceClass = 'confidence-ok';
  ...
  if (issues.length) els.profitWarning.textContent = `Check: ${issues.join(', ')}.`;
  else if (actualProfit < 0) els.profitWarning.textContent = `Risky quote: profit is ${money(actualProfit)} and margin is ${marginPct.toFixed(1)}%. This quote is below break-even.`;
  else if (marginPct < 15) els.profitWarning.textContent = `Risky quote: margin is only ${marginPct.toFixed(1)}%. Consider raising the price substantially.`;
  else if (actualProfit < 5 || marginPct < 25) els.profitWarning.textContent = `Low margin: profit is ${money(actualProfit)} and margin is ${marginPct.toFixed(1)}%. Consider increasing price before sending.`;
  else els.profitWarning.textContent = 'Quote looks ready to send.';
...
function getRowsData(container) {
  return [...container.children].map(row => ({ label: row.querySelector('.item-label').value, type: row.querySelector('.item-type').value, amount: num(row.querySelector('.item-amount').value), note: row.querySelector('.item-note').value }));
}
function snapshotQuote() {
  return {
    quoteNumber: els.quoteNumber.value, invoiceNumber: els.invoiceNumber.value, quoteDate: els.quoteDate.value, invoiceDate: els.invoiceDate.value, validThrough: els.validThrough.value, paymentDueDate: els.paymentDueDate.value,
    turnaround: els.turnaround.value, quoteTitle: els.quoteTitle.value, customerName: els.customerName.value, customerEmail: els.customerEmail.value, companyName: els.companyName.value, contactName: els.contactName.value, poNumber: els.poNumber.value,
    qty: els.qty.value, unitsPerItem: els.unitsPerItem.value, professionalMode: els.professionalMode.value, invoiceType: els.invoiceType.value, paymentTerms: els.paymentTerms.value, presetSelect: els.presetSelect.value, depositPercent: els.depositPercent.value,
    quoteStatus: els.quoteStatus.value, quoteNotes: els.quoteNotes.value, customerNotes: els.customerNotes.value, assumptions: els.assumptions.value, invoiceNotes: els.invoiceNotes.value, suggestedMode: els.suggestedMode.value, profitMode: els.profitMode.value,
    profitValue: els.profitValue.value, discount: els.discount.value, taxPreset: els.taxPreset.value, salesTax: els.salesTax.value, roundingMode: els.roundingMode.value, orderType: els.orderType.value, shippingMode: els.shippingMode.value, filamentCount: els.filamentCount.value,
    spoolWeight: els.spoolWeight.value, filament1Cost: els.filament1Cost.value, filament1Used: els.filament1Used.value, filament2Cost: els.filament2Cost.value, filament2Used: els.filament2Used.value, filament3Cost: els.filament3Cost.value, filament3Used: els.filament3Used.value,
    filament4Cost: els.filament4Cost.value, filament4Used: els.filament4Used.value, simplePackaging: els.simplePackaging.value, simpleShipping: els.simpleShipping.value, simpleHardware: els.simpleHardware.value, designHours: els.designHours.value, designRate: els.designRate.value,
    postHours: els.postHours.value, postRate: els.postRate.value, machineHours: els.machineHours.value, machineRate: els.machineRate.value, marketplacePercent: els.marketplacePercent.value, batchName: els.batchName.value, batchSku: els.batchSku.value, batchUnits: els.batchUnits.value,
    batchColors: els.batchColors.value, batchRollWeight: els.batchRollWeight.value, batchPriceTarget: els.batchPriceTarget.value, batchFilament1Cost: els.batchFilament1Cost.value, batchFilament1Used: els.batchFilament1Used.value, batchFilament2Cost: els.batchFilament2Cost.value,
    batchFilament2Used: els.batchFilament2Used.value, batchFilament3Cost: els.batchFilament3Cost.value, batchFilament3Used: els.batchFilament3Used.value, batchFilament4Cost: els.batchFilament4Cost.value, batchFilament4Used: els.batchFilament4Used.value, batchPackaging: els.batchPackaging.value,
    batchLabor: els.batchLabor.value, batchOther: els.batchOther.value, batchOverhead: els.batchOverhead.value, batchSold: els.batchSold.value, batchLocation: els.batchLocation.value, directItems: getRowsData(els.directItems), overheadItems: getRowsData(els.overheadItems), savedAt: new Date().toISOString()
  };
}
...
async function saveQuote() {
  ...
  try {
    ...
    } else els.saveQuoteBtn.textContent = 'Saved';
  } catch (err) {
...
async function copyFinance() {
  ...
  } catch { alert('Could not copy finance entry.'); }
}
function generateCustomerPdf() {
  render();
  fillPdf('quote', textMoneyToNumber(els.outFinal.textContent), textMoneyToNumber(els.outRoundedBeforeTax.textContent), textMoneyToNumber(els.outTax.textContent), textMoneyToNumber(els.outDeposit.textContent), textMoneyToNumber(els.outBalance.textContent), textMoneyToNumber(els.outPerItem.textContent));
  window.print();
  setTimeout(() => { forceNormalScreenView(); render(); }, 700);
}
function generateInvoicePdf() {
  render();
  fillPdf('invoice', textMoneyToNumber(els.outFinal.textContent), textMoneyToNumber(els.outRoundedBeforeTax.textContent), textMoneyToNumber(els.outTax.textContent), textMoneyToNumber(els.outDeposit.textContent), textMoneyToNumber(els.outBalance.textContent), textMoneyToNumber(els.outPerItem.textContent));
  window.print();
  setTimeout(() => { forceNormalScreenView(); render(); }, 700);
}
...
async function resetPage() {
  ...
  els.orderType.value = 'custom'; els.shippingMode.value = 'pickup'; els.quoteStatus.value = 'pending'; els.quoteDate.value = today(); els.invoiceDate.value = today(); els.validThrough.value = addDays(today(), 14); els.paymentDueDate.value = addDays(today(), 14); els.qty.value = 1; els.unitsPerItem.value = 1; els.presetSelect.value = 'custom'; els.depositPercent.value = 50; els.profitMode.value = 'percent'; els.profitValue.value = 30; els.discount.value = 0; els.taxPreset.value = 'portage'; els.salesTax.value = 7.25; els.roundingMode.value = 'none'; els.suggestedMode.value = 'off'; els.professionalMode.value = 'off'; els.invoiceType.value = 'deposit'; els.paymentTerms.value = 'deposit_to_start';
  ...
  els.filamentCount.value = 1; els.spoolWeight.value = 1000; els.batchRollWeight.value = 1000; els.batchColors.value = 1; els.batchUnits.value = 1; els.batchSold.value = 0; els.directItems.innerHTML = ''; els.overheadItems.innerHTML = '';
  clearMissingHighlights(); showMissingInputsNotice([]); applyOrderType(); applyTaxPreset(); refreshHistoryUI(); await ensureDocumentNumbers(true); await seedDefaults(); forceNormalScreenView();
}
async function loadDemo() {
  await resetPage();
  els.quoteNumber.value = 'Q-001042'; els.invoiceNumber.value = 'INV-001042'; els.professionalMode.value = 'on'; els.invoiceType.value = 'deposit'; els.orderType.value = 'business_bulk'; els.shippingMode.value = 'ship_estimated'; els.quoteTitle.value = '3200-piece branded PETG spacer order'; els.companyName.value = 'Sample Manufacturing Co.'; els.contactName.value = 'Dana Morris'; els.customerName.value = 'Dana Morris'; els.customerEmail.value = 'purchasing@example.com'; els.poNumber.value = 'PO-88214'; els.turnaround.value = '2–3 production weeks after approval'; els.qty.value = 3200; els.profitValue.value = 25; els.salesTax.value = 0; els.taxPreset.value = 'out_of_state'; els.depositPercent.value = 50; els.customerNotes.value = 'Quote includes production of the listed quantity using the discussed geometry and material assumptions. Packaging and delivery timing to be confirmed at award.'; els.invoiceNotes.value = 'Please reference the invoice number and PO number with remittance.'; els.assumptions.value = defaultAssumptions();
  els.filamentCount.value = 1; els.filament1Cost.value = 23; els.filament1Used.value = 5120; els.simplePackaging.value = 40; els.simpleShipping.value = 0; els.simpleHardware.value = 0; els.designHours.value = 1; els.designRate.value = 75; els.machineHours.value = 192; els.machineRate.value = 3.75; els.postHours.value = 4; els.postRate.value = 20;
  render(); forceNormalScreenView();
}
