...
| `finance_correction_receipts` | RLS enabled; owner SELECT policy only; authenticated has column SELECT; visible mutation grants belong to `service_role`; no mutation trigger | Owner-readable receipt; writes controlled by correction command/service role; browser mutation unavailable | **GREEN for supplied authenticated column grants; YELLOW overall** | The supplied columns support the intended read-only authenticated surface. Still prove table-level INSERT/UPDATE/DELETE absence, complete column grid, and correction-function grants/body. |
| `financial_entries` | RLS enabled; permissive owner UPDATE policy; authenticated has 31 live column UPDATE grants, including `amount`, tax, shipping, costs, title/type, and `updated_at`; no BEFORE UPDATE immutability trigger was present in the supplied all-trigger topology | Owner read and narrow manual creation; manual changes only through guarded RPC; posted Order income and corrections append-only | **RED — P0 correctness/control defect** | In PostgreSQL a column-level UPDATE grant is sufficient for an UPDATE limited to granted columns; a separate table-level UPDATE grant is not required. The owner UPDATE policy permits the authenticated owner row, and the supplied topology has no UPDATE guard that distinguishes manual from command-owned rows. An authenticated client can therefore change financially authoritative values on its own command-owned posted income row outside the correction command. |

The three `financial_entries` INSERT triggers are consistent with separate eligibility, accepted-invoice, and tax-metadata responsibilities; their coexistence is not presently classified as competing authority because all are BEFORE INSERT guards/enrichment. None protects UPDATE. The repository browser currently uses `update_manual_financial_entry` and `correct_financial_entry` RPCs rather than issuing a direct `financial_entries` PATCH, which is good client behavior but not a database security boundary: any authenticated browser session can call the REST table endpoint directly with its token.

**Exact risk:** an owner can bypass correction receipts, correction reason, idempotency, original-row preservation, and effective-entry reporting by directly changing granted economic fields on a posted Order income row. This can silently alter revenue, collected sales tax, shipping, costs, and profitability while leaving `finance_pushed`, the primary posting identity, and correction audit history apparently valid. Owner scoping prevents cross-owner mutation but does not preserve ledger correctness. This is currently **P0** because the necessary privilege, RLS path, economically significant columns, and absence of an UPDATE guard are all evidenced live; no mutation probe is needed or permitted.

**Remaining evidence (scope, not RED determination):** return `19_TABLE_GRANTS` and the updated `20_IMMUTABILITY` effective privilege booleans to establish DELETE and whole-table privilege scope; the complete `20_IMMUTABILITY_COLUMN_GRANTS` export to confirm whether command identity/linkage columns are also writable; `20_IMMUTABILITY_GUARD_FUNCTIONS`/section 06 to archive full deployed trigger bodies; sections 03–05 for `update_manual_financial_entry`, `correct_financial_entry`, and correction receipt writers; and a read-only count/ID scan for command-owned rows (not customer fields) to quantify exposure. These results can increase scope or identify additional defects, but cannot negate the already-proven direct UPDATE path unless the supplied live evidence came from different environments or snapshots.
...
| `financial_entries` authenticated UPDATE | Live: 31 column UPDATE grants, owner UPDATE policy, no UPDATE guard | Manual editing through guarded RPC; command-owned rows immutable | **RED P0:** posted income/tax/cost can change without correction evidence | GRANT CORRECTION (forward-only migration); POLICY REPLACEMENT/immutability guard as defense in depth |
...
| **P0.1a — confirmed** | Live authenticated direct UPDATE path to command-owned `financial_entries` economic columns | SMALL–MEDIUM | HIGH | GRANT CORRECTION; consider POLICY REPLACEMENT/immutable-row guard |
