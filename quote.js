...
(() => {
  ...
  function renderStandaloneQuote() {
    return typeof window.olipolySyncQuoteTotals === "function"
...
/* === Milestone 1A: authoritative customer pricing adapter === */
(() => {
  const $ = (id) => document.getElementById(id);
  const value = (id) => String($(id)?.value ?? '');
  const number = (id) => Number(value(id).replace(/[^0-9.-]/g, '')) || 0;
  const money = (amount) => new Intl.NumberFormat('en-US', {style:'currency', currency:'USD'}).format(Number(amount) || 0);
  const setText = (id, text) => document.querySelectorAll(`#${id}, [data-sync-id="${id}"]`).forEach((el) => { el.textContent = text; });

  function readQuotePricingInput(){
    return {
      quantity:number('qty') || number('quantity') || 1,
      manualPiecePrice:value('manualPiecePriceOverride').trim(),
      suggestedTotal:number('productionSuggestedTotal'),
      suggestedPiecePrice:number('productionSuggestedPiecePrice'),
      material:number('productionMaterialCost'), machine:number('productionMachineCost'),
      design:number('productionDesignCost'), post:number('productionPostCost'),
      packaging:number('productionPackagingCost'), shipping:number('productionShippingCost'),
      hardware:number('productionHardwareCost'), directCost:number('productionDirectCost'),
      overheadCost:number('productionOverheadCost'), breakEven:number('productionBreakEven'),
      discount:number('discount'), taxExempt:value('taxExempt') === 'yes',
  ...
  function getQuoteTotals(){
    const totals = window.calculateQuoteTotals(readQuotePricingInput());
    return {...totals, finalText:money(totals.total), totalText:money(totals.total), subtotalText:money(totals.subtotal),
      taxText:money(totals.tax), perItemText:money(totals.perItem), piecePriceText:money(totals.perItem),
      depositText:money(totals.deposit), balanceText:money(totals.balance)};
  }
  ...
  function renderQuotePricing(){
    ...
    if(notice){
      notice.classList.toggle('hidden',!t.hasManualPrice); notice.classList.toggle('manual-override-active',t.hasManualPrice);
      if(t.hasManualPrice){const reason=value('manualPiecePriceReason').trim(); notice.innerHTML=t.manualPiecePrice===0
        ? `<strong>Complimentary quote:</strong> Customer price is intentionally $0.00.${reason?`<br><strong>Reason:</strong> ${reason}`:''}`
        : `<strong>Custom selling price:</strong> ${money(t.manualPiecePrice)} × ${t.quantity} before discount and tax.${reason?`<br><strong>Reason:</strong> ${reason}`:''}`;}
    }
    window.olipolyQuoteTotals=t;
    window.olipolyRenderedQuoteTotals={qty:t.quantity,pricingMode:t.pricingMode,perItem:t.perItem,subtotal:t.subtotal,tax:t.tax,
      deposit:t.deposit,balance:t.balance,total:t.total,perItemText:t.perItemText,subtotalText:t.subtotalText,taxText:t.taxText,
      depositText:t.depositText,balanceText:t.balanceText,totalText:t.totalText,capturedAt:new Date().toISOString()};
    window.olipolyLastSyncedQuoteTotals=t;
    return t;
  }

  window.olipolyGetQuoteTotals=getQuoteTotals;
...
/* === OliPoly compact quote PDF styling V4 === */
(() => {
  ...
  if(document.readyState === "loading") document.addEventListener("DOMContentLoaded", injectCompactPdfCss);
  else injectCompactPdfCss();
})();
...
(() => {
  ...
  const $ = (id) => document.getElementById(id);
  const params = new URLSearchParams(window.location.search);
  const PRODUCTION_ESTIMATE_FIELDS = {
    productionSuggestedTotal:'suggested_total',
    productionSuggestedPiecePrice:'suggested_piece_price',
    ...
    productionPostCost:'estimated_post_cost',
    productionPackagingCost:'estimated_packaging_cost',
    productionShippingCost:'estimated_shipping_cost',
    productionHardwareCost:'estimated_supply_cost',
    productionDirectCost:'estimated_direct_cost',
    productionOverheadCost:'estimated_overhead_cost',
    productionBreakEven:'estimated_break_even'
  };
  ...
  function applyProductionEstimateValues(draft){
    if(!draft) return;
    Object.entries(PRODUCTION_ESTIMATE_FIELDS).forEach(([fieldId,draftKey]) => {
      ensureHidden(fieldId).value = draft[draftKey] ?? '';
    });
  }
  ...
  function applyProductionDraft(){
    ...
    applyProductionEstimateValues(draft);
    ...
    setTimeout(() => applyProductionIdentity(draft), 250);
    setTimeout(() => applyProductionIdentity(draft), 1000);
  ...
  function run(){
    addPresentationStyles();
    movePricingFields();
    Object.keys(PRODUCTION_ESTIMATE_FIELDS).forEach(ensureHidden);
