...
-- Normal Orders Admin edits cross one command boundary. Browser roles retain no
-- direct UPDATE authority on orders; lifecycle, payment, invoice, Finance,
-- accepted-commercial and audit columns cannot be named by this command.
...
declare
  ...
  v_allowed constant text[] := array[
    ...
    'ap_email','billing_address','shipping_address','internal_notes'
  ];
  v_rejected text[];
begin
  ...
  select * into v_order
  from public.orders
  where id = p_order_id and user_id = v_actor
  for update;
  if not found then
