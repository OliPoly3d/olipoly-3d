...
-- Serialize pre-acceptance work by Production job before any blocking row lock.
-- Command identity remains independently protected for receipt/idempotency.
-- This does not weaken the authoritative owner, evidence, row-lock, receipt,
-- optimistic-concurrency, RLS, or grant contract.
...
begin
  ...
  if not pg_try_advisory_xact_lock(v_job_lock_key) then
    raise exception 'Another Quote handoff is already operating on this Production job. Refresh before retrying.' using errcode='55P03';
  end if;
  ...
  if not pg_try_advisory_xact_lock(v_command_lock_key) then
    raise exception 'Quote handoff command identity is already in progress; refresh before retrying.' using errcode='55P03';
  end if;
