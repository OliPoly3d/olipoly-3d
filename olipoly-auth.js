...
(function(){
  ...
  const getUser=async()=>{
    ...
    const res=await fetch(`${SUPABASE_URL}/auth/v1/user`,{method:'GET',headers:{apikey:SUPABASE_KEY,Authorization:`Bearer ${token}`}});
    const data=await res.json().catch(()=>null);
  ...
  const refresh=async()=>{
    ...
    try{
      const data=await api('/auth/v1/token?grant_type=refresh_token',{method:'POST',body:JSON.stringify({refresh_token:refreshToken})});
      return writeSession(data);
    }catch(err){
  ...
  const ensure=async()=>{
    ...
    if(current.access_token && !current.user){
      ...
      if(u) return writeSession({...current,user:u});
    }
    return current;
  };
  window.OliPolyAuth={
    SUPABASE_URL,SUPABASE_KEY,SESSION_KEY,TOKEN_KEY,REFRESH_KEY,USER_KEY,
    login,signup,logout:clearSession,clearSession,readSession,writeSession,refresh,ensure,getToken,getUser,
    authHeaders(){const t=getToken();return {apikey:SUPABASE_KEY,'Content-Type':'application/json',...(t?{Authorization:`Bearer ${t}`}:{})};}
  };
  ensure();
})();
