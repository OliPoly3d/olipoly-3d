-- Fast-contention safety for Production workflow mutations.
-- Client single-dispatch is primary; these locks bound accidental concurrent callers.
...
comment on function public.production_workflow_command(text,text,timestamptz,jsonb,text,text) is 'Atomic Production/Order lifecycle authority. Same command identities serialize for receipt reconciliation; different concurrent actions fail through NOWAIT row locks; lock waits are bounded to two seconds.';
