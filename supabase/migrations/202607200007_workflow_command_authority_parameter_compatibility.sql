-- Purpose and scope:
-- Forward-only corrective replacement for failed, merged migration 202607200006.
-- Migration 006 was not deployed because PostgreSQL rejected changing the
-- deployed set_linked_workflow_status(text,text,timestamptz) argument names.
-- This migration intentionally preserves migration 006 behavior while keeping
-- the deployed legacy function parameter names in its retirement wrapper.
--
...
--
-- Deployment order:
-- Deploy after 202607200005_quote_acceptance_runtime_safety.sql. Do not execute
-- 202607200006_workflow_command_authority.sql; it failed before deployment and
-- this file supersedes it for the workflow-command-authority correction. This
-- migration must be reviewed and applied by an operator through the approved
-- Supabase migration process. Codex did not apply or deploy this migration.
--
