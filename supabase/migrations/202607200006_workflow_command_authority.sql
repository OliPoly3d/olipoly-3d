...
create or replace function public.workflow_actor_id()
returns text language sql stable as $$ select nullif(auth.uid()::text,'') $$;
...
begin
  ...
  if v_command = 'start_print' then
    ...
    v_actual_grams := nullif(p_payload->>'actual_grams_used','')::numeric;
    v_scrap_grams := coalesce(nullif(p_payload->>'scrap_grams','')::numeric, 0);
    v_actual_hours := nullif(p_payload->>'actual_print_hours','')::numeric;
    if v_actual_grams is null or v_actual_grams < 0 or v_scrap_grams < 0 or v_actual_hours is null or v_actual_hours < 0 then raise exception 'Complete Print requires nonnegative actual_grams_used, scrap_grams, and actual_print_hours' using errcode='22023'; end if;
    v_to := 'qc'; v_event := 'order.print_completed';
  elsif v_command = 'pass_qc' then
    if v_job.production_status <> 'qc' then raise exception 'Pass QC requires qc' using errcode='22023'; end if;
  ...
  update public.production_jobs
     set production_status = v_to,
         ...
         actuals_captured_at = case when v_command='complete_print' then v_now else actuals_captured_at end,
  ...
  values(gen_random_uuid(),v_actor,v_order.source_quote_number,v_order.order_number,v_event,jsonb_build_object('from',v_job.production_status,'to',v_to),v_now,v_now,'order',v_order.order_number,'authenticated_user',v_actor::text,v_correlation,p_causation_id,1,jsonb_build_object('command',v_command,'status',v_to,'production_job_id',v_job.id,'actuals',coalesce(p_payload,'{}'::jsonb)))
...
begin
  ...
  insert into public.project_events(event_id,user_id,quote_number,order_number,event_type,details,created_at,occurred_at,aggregate_type,aggregate_id,actor_type,actor_id,correlation_id,causation_id,schema_version,payload)
  values(gen_random_uuid(),v_actor,v_order.source_quote_number,v_order.order_number,v_event,jsonb_build_object('status',v_to),v_now,v_now,'order',v_order.order_number,'authenticated_user',v_actor::text,v_correlation,p_causation_id,1,jsonb_build_object('command',v_command,'status',v_to,'fulfillment',coalesce(p_payload,'{}'::jsonb)))
  on conflict (correlation_id, event_type) where correlation_id is not null and event_type in ('order.printing_started','order.print_completed','order.qc_passed','order.needs_reprint','order.ready_to_print','order.closed') do nothing;
  return v_order;
end;
$$;
...
grant select on table public.orders, public.production_jobs, public.order_tracking_public to authenticated;
