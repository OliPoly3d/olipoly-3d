...
async function syncAcceptedQuoteToOrders() {
  ...
  if (orderRes.error) {
    console.error('orders sync failed', orderRes.error);
    throw new Error(orderRes.error.message || JSON.stringify(orderRes.error) || 'Could not create or update the order.');
  }

  const publicPayload = buildAcceptedQuotePublicTrackingPayload(orderPayload);
  ...
  if (publicRes.error) {
    console.error('public tracking sync failed', publicRes.error);
...
els.saveQuoteBtn.onclick = async () => { await saveQuote(); };
