-- Repository reconciliation for the manually repaired authoritative Inventory consumption deployment.
-- Forward-only and idempotent: safe after the operator-applied repair and safe for a fresh
-- sequential repository environment that needs the verified repair after the failed historical artifact.
--
-- IMPORTANT OPERATOR NOTE:
-- The operator already applied the equivalent repair manually in the deployed Supabase project.
-- Do not manually rerun this reconciliation SQL solely for deployment; keep this migration as the
-- repository record that reconciles future environments with the deployed contract.
--
-- Do not modify the previously merged broken migration. Historical ledger rows are preserved;
-- only missing columns/defaults and NULL occurred_at values are reconciled.
...
create unique index if not exists inventory_transactions_production_attempt_roll_once
  ...
  where transaction_type = 'production_attempt_consumption'
    and production_job_id is not null
    and raw_material_id is not null
    and attempt_id is not null;

create unique index if not exists inventory_transactions_production_command_roll_once
  on public.inventory_transactions (user_id, correlation_id, raw_material_id)
  where transaction_type = 'production_attempt_consumption'
...
-- Consolidated read-only verification query (operator): does not execute the RPC and does not call
-- has_function_privilege with 'PUBLIC'. ACL inspection is used for PUBLIC/role execute posture.
-- select jsonb_build_object(
--   'ledger_columns', (
--     select jsonb_object_agg(c.column_name, jsonb_build_object('data_type', c.data_type, 'column_default', c.column_default) order by c.column_name)
--     from information_schema.columns c
--     where c.table_schema = 'public'
--       and c.table_name = 'inventory_transactions'
--       and c.column_name in ('occurred_at','attempt_id','correlation_id','quantity_grams','order_number','quote_number')
--   ),
--   'occurred_at_null_count', (select count(*) from public.inventory_transactions where occurred_at is null),
--   'authoritative_rows', (select count(*) from public.inventory_transactions where transaction_type = 'production_attempt_consumption'),
--   'indexes', (
--     select jsonb_object_agg(i.indexname, i.indexdef order by i.indexname)
--     from pg_indexes i
--     where i.schemaname = 'public'
--       and i.tablename = 'inventory_transactions'
--       and i.indexname in ('inventory_transactions_production_attempt_roll_once','inventory_transactions_production_command_roll_once')
--   ),
--   'rejected_correlation_only_index_absent', not exists (
--     select 1 from pg_indexes i
--     where i.schemaname = 'public'
--       and i.tablename = 'inventory_transactions'
--       and i.indexname = 'inventory_transactions_production_command_once'
--   ),
--   'rpc', (
--     select jsonb_build_object(
...
--           and acl.privilege_type = 'EXECUTE'
--       ),
--       'remaining_grams_raw_quantity_authority', pg_get_functiondef(p.oid) like '%remaining_grams = remaining_grams - v_grams%',
--       'does_not_update_current_grams', pg_get_functiondef(p.oid) not like '%current_grams%',
--       'retry_before_concurrency', strpos(pg_get_functiondef(p.oid), 'if v_existing is not null') < strpos(pg_get_functiondef(p.oid), 'updated_at is distinct from p_expected_updated_at')
--     )
--     from pg_proc p
--     join pg_namespace n on n.oid = p.pronamespace
--     where n.nspname = 'public'
--       and p.proname = 'consume_production_attempt'
--       and pg_get_function_identity_arguments(p.oid) = 'p_production_job_id uuid, p_attempt_id text, p_correlation_id text, p_expected_updated_at timestamp with time zone, p_roll_usages jsonb, p_workflow_command text'
--   )
-- ) as inventory_consumption_reconciliation_verification;
