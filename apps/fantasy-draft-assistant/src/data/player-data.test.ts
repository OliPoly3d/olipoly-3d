...
describe('truthful bye-week normalization',()=>{
  it('accepts explicit NFL weeks and rejects missing or implausible values',()=>{
  ...
  it('preserves an explicit provider bye in both league player semantics',()=>{
    for(const slug of ['believeland','robocop'] as const){
      const format:'HALF_PPR'|'PPR'=slug==='robocop'?'HALF_PPR':'PPR';
      const active={...snapshot([intel('synthetic-1',{byeWeek:11})]),scoringFormat:format};
      expect(selectPlayerPool(playerPool(),active)[0].byeWeek).toBe(11);
    }
  });
});
