-- TEMPORARY DIAGNOSTIC ONLY. Apply manually, capture one request, then deploy migration 202608030003 to remove logging.
...
begin
  ...
  raise log 'OP_WORKFLOW stage=ORDER_FOUND correlation=% order=% job=%',v_command_id,p_order_number,null;
  ...
  raise log 'OP_WORKFLOW stage=PRODUCTION_FOUND correlation=% order=% job=%',v_command_id,p_order_number,v_job.id;
...
declare v_correlation text:=current_setting('olipoly.workflow_correlation',true);
begin
  raise log 'OP_WORKFLOW trigger=normalize_status stage=TRIGGER_ENTER correlation=% table=%',v_correlation,tg_table_name;
  new.status:=public.normalize_accepted_order_status(new.status);
  raise log 'OP_WORKFLOW trigger=normalize_status stage=TRIGGER_EXIT correlation=% table=%',v_correlation,tg_table_name;
  return new;
end $$;
...
declare v_correlation text:=current_setting('olipoly.workflow_correlation',true);
begin
  raise log 'OP_WORKFLOW trigger=set_orders_updated_at stage=TRIGGER_ENTER correlation=% table=%',v_correlation,tg_table_name;
  new.updated_at:=now();
  raise log 'OP_WORKFLOW trigger=set_orders_updated_at stage=TRIGGER_EXIT correlation=% table=%',v_correlation,tg_table_name;
  return new;
end $$;
...
declare v_correlation text:=current_setting('olipoly.workflow_correlation',true);
begin
  raise log 'OP_WORKFLOW trigger=legacy_order_sync stage=TRIGGER_ENTER correlation=% table=%',v_correlation,tg_table_name;
  ...
  raise log 'OP_WORKFLOW trigger=legacy_order_sync stage=TRIGGER_EXIT correlation=% table=%',v_correlation,tg_table_name;
  return new;
end $$;
