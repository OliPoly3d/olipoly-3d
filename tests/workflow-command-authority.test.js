...
assert(migration.includes('command_identity text primary key') && !migration.includes('workflow_command_receipts_identity_idx'), 'command receipt primary key must provide uniqueness without a redundant unique index');
...
assert(migration.includes('return v_job;') && migration.includes('resulting_updated_at') && migration.includes('result_snapshot jsonb not null'), 'same-command receipt retry must return the committed Production row and retain immutable result evidence');
assert(!migration.includes('v_job.updated_at is distinct from v_receipt.resulting_updated_at'), 'valid pre-acceptance retry must not fail because unrelated permitted edits changed updated_at');
assert(migration.includes('alter table if exists public.workflow_command_receipts enable row level security'), 'command receipts must explicitly enable RLS');
...
assert(migration.includes("has_table_privilege('PUBLIC','public.workflow_command_receipts','select')") && migration.includes("relrowsecurity from pg_class where oid='public.workflow_command_receipts'::regclass"), 'receipt RLS and effective privilege verification queries must be present');
assert(!/create policy .* on public\.workflow_command_receipts .*to authenticated/i.test(migration) && !/create policy .* on public\.workflow_command_receipts .*to anon/i.test(migration), 'receipt table must create no browser policies');
...
assert(production.includes('const authoritative = atomicConsumption?.production_job || await syncProductionStatusToOrder(j, status, j, commandContext)'), 'success must consume the atomic QC row or dispatch once with the unchanged action identity');
...
assert(production.includes("if(j.order_number || j.production_source_type === 'legacy_standalone'){") && production.includes('Unsupported linked workflow transition'), 'linked and explicitly classified legacy work route through command authority before local mutation');
...
assert(/consumeCapturedAttempt\(updated,[\s\S]*workflowCommand:status === 'ready_to_print' \? 'needs_reprint' : 'pass_qc'[\s\S]*atomicConsumption\?\.production_job \|\| await syncProductionStatusToOrder/.test(production), 'QC consumption and workflow progression share the atomic result; non-consumption commands dispatch once');
assert(/lifecycleAction === 'consume_and_reserve_reprint'[\s\S]*workflowCommand:status === 'ready_to_print' \? 'needs_reprint' : 'pass_qc'[\s\S]*reserveLinkedProductionMaterial/.test(production), 'linked Needs Reprint atomically consumes the attempt and reaches the authoritative re-reservation path');
assert(production.includes('LINKED_WORKFLOW_RECOVERY_KEY') && production.includes('rememberLinkedWorkflowRecovery') && production.includes('pendingLinkedWorkflowRecovery'), 'linked Inventory/RPC orchestration must record durable browser recovery state');
assert(/pendingLinkedWorkflowRecovery\(j, status\)[\s\S]*if\(!recovery\)[\s\S]*rememberLinkedWorkflowRecovery\(j, status, updated, lifecycleAction\)[\s\S]*atomicConsumption\?\.production_job \|\| await syncProductionStatusToOrder\(j, status, j, commandContext\)[\s\S]*clearLinkedWorkflowRecovery\(id\)/.test(production), 'retry reuses pending non-atomic Inventory work; recovery clears only after an atomic result or authoritative command success');
assert(production.includes('Linked accepted work cannot be moved back directly') && production.includes("sbApi('/rest/v1/rpc/cancel_production_job'"), 'move-back is blocked and cancellation uses its authoritative command instead of a direct save');
...
assert(orders.includes('const ordinaryPayload = buildOrdinaryOrderEditPayload(payload, currentRow);'), 'Orders Admin ordinary saves must use the changed-field editable allowlist builder');
...
for (const editableColumn of ['customer_name','order_title','tracking_number','destination_county','sales_tax_rate','internal_notes']) {
...
for (const protectedColumn of [
  ...
  'payment_status','paid_date','invoice_number','updated_at','completion_email_sent','completion_email_sent_at','catalog_part_id'
]) {
...
assert(orders.includes("fulfillmentWorkflowRpcRequest(order.order_number,'close_order',order.updated_at") && orders.includes('Close command did not return the authoritative Closed Order.'), 'Orders close uses the hydrated row version and consumes the command-returned authoritative row');
