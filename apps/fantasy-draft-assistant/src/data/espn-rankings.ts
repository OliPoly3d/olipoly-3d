...
export interface ExtractedPdf { text:string; pageCount:number; pagesProcessed:number; pageTextItemCounts:number[]; title?:string; createdAt?:Date }
...
export async function extractPdfDocument(data:ArrayBuffer,onProgress?:(page:number,total:number)=>void):Promise<ExtractedPdf>{
  const pages:string[]=[],pageTextItemCounts:number[]=[];
  try{
    ...
    for(let pageNumber=1;pageNumber<=document.numPages;pageNumber++){const page=await document.getPage(pageNumber),content=await page.getTextContent();pageTextItemCounts.push(content.items.length);pages.push(content.items.map(item=>'str' in item?`${item.str}${'hasEOL' in item&&item.hasEOL?'\n':' '}`:'').join(''));onProgress?.(pageNumber,document.numPages)}
    const text=pages.join('\n\f\n').replace(/[ \t]+/g,' ').replace(/\n +/g,'\n').trim();if(!text)throw new Error('IMAGE_ONLY');
    return{text,pageCount:document.numPages,pagesProcessed:pages.length,pageTextItemCounts,title:typeof info?.Title==='string'?info.Title:undefined,createdAt:pdfDate(info?.CreationDate)};
  }catch(error){
...
export function parseEspnPdfText(text:string):{rows:EspnImportRow[];invalid:EspnImportPreview['invalid']} {
  ...
  const exactRows=new Set<string>(),uniqueRows:EspnImportRow[]=[];for(const row of rows){const identity=JSON.stringify(row);if(exactRows.has(identity))continue;exactRows.add(identity);uniqueRows.push(row)}
  const seen=new Set<number>();for(const row of uniqueRows)if(seen.has(row.overall_rank))invalid.push({row:row.overall_rank,detectedRank:row.overall_rank,rawText:`${row.overall_rank}. (${row.position}${row.position_rank??'?'}) ${row.player_name}, ${row.team??'—'}`,reason:`Duplicate overall rank ${row.overall_rank}.`,sourceRow:row});else seen.add(row.overall_rank);
  ...
  return{rows:uniqueRows.sort((a,b)=>a.overall_rank-b.overall_rank),invalid};
}
...
export function reconcileEspnRows(rows:EspnImportRow[],players:DraftPlayer[],metadata:EspnImportMetadata,invalid:EspnImportPreview['invalid']=[]):EspnImportPreview{
  ...
  for(const row of rows){const name=normalizePlayerName(row.player_name),position=normalizePosition(row.position);if(!Number.isInteger(row.overall_rank)||row.overall_rank<1||row.overall_rank>300||!fantasyPositions.has(position)||!name){invalid.push({row:row.overall_rank,detectedRank:Number.isFinite(row.overall_rank)?row.overall_rank:undefined,rawText:JSON.stringify(row),reason:'Invalid rank, name, or position.'});continue}const candidates=canonicalRankingCandidates({name:row.player_name,team:row.team,position:row.position},players,aliases[name]?[aliases[name]]:[]);if(candidates.length===1)matched.push({row,player:candidates[0]});else if(candidates.length>1)ambiguous.push({row,candidates});else unmatched.push(row)}
...
export const isEspnImportActivatable=(preview:EspnImportPreview,leagueScoring:ScoringFormat)=>(void leagueScoring,preview.rows.length>0&&!preview.invalid.length&&!preview.unmatched.length&&!preview.ambiguous.length&&!duplicateAssignments(preview)&&preview.matched.length>0);
export function espnActivationError(preview:EspnImportPreview,leagueScoring:ScoringFormat):string|undefined{void leagueScoring;if(preview.invalid.length)return`${preview.invalid.length} invalid row${preview.invalid.length===1?'':'s'} must be resolved or ignored before activation.`;const unresolved=preview.unmatched.length+preview.ambiguous.length;if(unresolved)return`${unresolved} unresolved ESPN row${unresolved===1?' remains.':'s remain.'}`;if(duplicateAssignments(preview))return'Duplicate canonical player assignments must be changed before activation.';if(!preview.rows.length||!preview.matched.length)return'At least one ESPN ranking must be matched before activation.';return undefined}
