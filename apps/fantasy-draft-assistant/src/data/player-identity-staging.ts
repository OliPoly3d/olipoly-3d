...
export interface StagedIdentityGroup{id:string;normalizedName:string;team?:string;position:string;evidence:IdentityEvidence[];candidates:DraftPlayer[];player?:DraftPlayer;disposition:IdentityDisposition;reason:string}
...
export interface IdentityStagingResult{groups:StagedIdentityGroup[];sources:ReconciledSource[];diagnostics:SourceIdentityDiagnostic[];blockingGroups:StagedIdentityGroup[];reviewGroups:StagedIdentityGroup[];informationalRows:number;repeatedAppearances:number;uniqueNormalizedPlayers:number;canonicalPoolSize:number}

/** Reconciles all uploads against the already-loaded canonical pool, then consolidates review by safe player evidence. */
export function stagePlayerIdentities(sources:ParsedSource[],players:DraftPlayer[]):IdentityStagingResult{
 const reconciled=sources.map(source=>reconcileDataCenterSource(source,players)),groups=new Map<string,StagedIdentityGroup>(),seenOccurrences=new Set<string>(),diagnostics:SourceIdentityDiagnostic[]=[];
 for(const source of reconciled){let duplicates=0,nonFantasy=0,outsidePool=0,informational=0;const unique=new Set<string>();
  ...
  for(const row of source.rows){const occurrence=occurrenceKey(row),duplicate=seenOccurrences.has(occurrence);seenOccurrences.add(occurrence);if(duplicate){duplicates++;continue}unique.add(identityKey(row));
   ...
   else groups.set(key,{id:key,normalizedName:normalizePlayerName(row.playerName),team:normalizeTeam(row.team),position:family(row.position),evidence:[evidence],candidates,player:matched,disposition,reason});
  }
 ...
 return{groups:all,sources:safeSources,diagnostics,blockingGroups,reviewGroups,informationalRows:all.filter(group=>group.disposition==='INFORMATIONAL_EXCLUDED').reduce((sum,group)=>sum+group.evidence.length,0),repeatedAppearances:sources.reduce((sum,source)=>sum+source.rows.length,0)-all.length,uniqueNormalizedPlayers:new Set(sources.flatMap(source=>source.rows.map(row=>normalizePlayerName(row.playerName))).filter(Boolean)).size,canonicalPoolSize:players.length};
}
