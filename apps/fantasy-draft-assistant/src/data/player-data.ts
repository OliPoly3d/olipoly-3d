...
/** NFL bye data is optional; only a plausible, explicit week may become authority. */
export const normalizeByeWeek=(value:unknown):number|undefined=>{if(typeof value==='string'&&!value.trim())return;const week=Number(value);return Number.isInteger(week)&&week>=1&&week<=18?week:undefined};
...
const supportedSnapshotPositions:ReadonlySet<Position>=new Set(['QB','RB','WR','TE','K','DST','DL','LB','DB','DT','DE','CB','S']);
...
export function selectPlayerPool(fixtures:DraftPlayer[],snapshot?:PlayerDataSnapshot):DraftPlayer[]{
  ...
  return snapshot.players.map(current=>({
    ...
    byeWeek:normalizeByeWeek(current.byeWeek),
