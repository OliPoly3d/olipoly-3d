...
begin
  ...
  if v_job.updated_at is distinct from p_expected_updated_at then
    raise exception 'Production workflow changed since this page loaded; refresh before retrying'
      using errcode='40001',
            detail=format('conflictScope=production_row expected=%s authoritative=%s jobId=%s orderNumber=%s',p_expected_updated_at,v_job.updated_at,v_job.id,v_order.order_number),
...
comment on function public.production_workflow_command(text,text,timestamptz,jsonb,text,text) is 'Atomic Production/Order lifecycle authority. p_expected_updated_at is the exact production_jobs.updated_at row version; Orders are locked and projected server-side.';
