...
const original = { id:'original', destination_county:null, sales_tax_rate:null, taxable_sales:20.50, sales_tax_collected:1.33, amount:20.50, shipping_charged:0, total:21.83 };
const receipts = [
  { created_at:'2026-08-01T10:00:00Z', status:'active', effective_record:{ destination_county:'Portage' } },
  { created_at:'2026-08-01T11:00:00Z', status:'voided', effective_record:{ destination_county:'Summit', sales_tax_rate:7.0 } },
  { created_at:'2026-08-01T12:00:00Z', status:'active', effective_record:{ ...original, destination_county:'Portage', sales_tax_rate:6.5 } }
];
const effective = receipts.filter(r => r.status !== 'voided').sort((a,b) => b.created_at.localeCompare(a.created_at))[0].effective_record;

assert.equal(original.destination_county, null, 'original remains unchanged');
assert.equal(original.sales_tax_rate, null, 'original rate remains unchanged');
assert.equal(effective.destination_county, 'Portage', 'county is overlaid');
assert.equal(effective.sales_tax_rate, 6.5, 'rate is overlaid');
assert.equal(effective.taxable_sales, 20.50, 'taxable sales carry forward');
...
assert.equal(+(effective.taxable_sales * effective.sales_tax_rate / 100).toFixed(2), 1.33, 'canonical tax reconciles');
assert.equal(receipts.filter(r => r.status !== 'voided').slice(-1).length, 1, 'effective projection contributes one transaction');
assert.equal(receipts.filter(r => r.status === 'voided')[0].effective_record.destination_county, 'Summit', 'voided correction is retained for audit but ignored');
...
assert.match(js, /Gross \/ Customer Total'[\s\S]*entry\.destination_county[\s\S]*entry\.sales_tax_rate[\s\S]*entry\.original_entry_id[\s\S]*entry\.effective_entry_id/, 'filing CSV uses effective tax values and preserves audit linkage');
