...
assert(/const recovery = pendingLinkedWorkflowRecovery\(j, status\)[\s\S]*?if\(!recovery\)\{[\s\S]*?consumeCapturedAttempt\(updated[\s\S]*?rememberLinkedWorkflowRecovery\(j, status, updated, lifecycleAction\)[\s\S]*?syncProductionStatusToOrder\(j, status\)[\s\S]*?clearLinkedWorkflowRecovery\(id\)/.test(production), 'RPC failure followed by retry must reuse pending Inventory work and clear recovery only after command success');
