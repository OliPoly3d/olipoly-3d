...
## Regression baseline reconciliation note (2026-07-21)

Status: **Repository test-baseline reconciliation; not deployed.**

This focused pass reconciled two recurring static-test failures without changing Supabase schema, RLS, grants, deployed state, historical records, or OP-000010.

- `tests/workflow-command-authority.test.js` root cause: **stale/brittle test assertion**. Orders Admin ordinary Save had already moved from an object-rest exclusion pattern to the ERP Blueprint v1-approved positive allowlist (`ORDERS_ADMIN_ORDINARY_EDIT_COLUMNS` via `buildOrdinaryOrderEditPayload`). The test still required the old destructuring string. The test now asserts the current architectural contract directly: ordinary Save must call the allowlist builder, PATCH only `ordinaryPayload`, keep direct creation disabled, keep known editable columns present, and keep identity/workflow/tracking/Quote/accepted-snapshot/Finance/action marker fields out of the ordinary PATCH payload.
- `tests/job-asset-model.test.js` root cause: **implementation integration regression**. Orders Admin retained the Job Assets stylesheet but lacked the stable `[data-job-assets]` mount and asset scripts used by the shared Job Files / Assets component. The smallest implementation repair restores that mount inside the existing Order Details card and loads `js/job-asset-model.js` plus `js/job-assets-ui.js`, preserving the private-bucket/owner-scoped Supabase authority and avoiding UI redesign.
- The final assertions match ERP Blueprint v1 because ordinary Order edits remain a narrow allowlisted browser PATCH while workflow transitions remain RPC-owned, and Job Files / Assets continues to use the shared private asset service rather than an Orders-specific duplicate implementation.
- Tests run: `node tests/workflow-command-authority.test.js`, `node tests/job-asset-model.test.js`, `node tests/orders-admin-action-regression.test.js`, related asset/workflow authority tests, inline JavaScript syntax extraction for `orders-admin.html`, `git diff --check`, and the full `tests/*.test.js` suite.
- Explicit non-actions: no SQL was deployed; no migrations were created; no Supabase state was altered; Finance, Inventory, Production, Quote acceptance, public payment/tracking, fundraiser behavior, and OP-000010/historical recovery evidence were not modified.
