...
assert.deepEqual(A.totalsRows(invoice).filter(r=>['Product subtotal','Sales tax','Accepted total','Total due'].includes(r[0])),[['Product subtotal',20.5],['Sales tax',1.33],['Accepted total',21.83],['Total due',21.83]]);
...
assert.deepEqual(A.totalsRows(aggregate).map(r=>r[0]),['Accepted order total','Deposit / prior payment','Amount paid','Total due']);
