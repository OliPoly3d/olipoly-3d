-- Run immediately after capturing one traced Start Print. This restores the non-logging authoritative definition.
-- Remove the retired bidirectional trigger from the final workflow execution graph.
-- production_workflow_command remains the sole post-acceptance mutation authority.
...
-- Fail deployment if any remaining user trigger can re-enter workflow authority.
do $guard$
declare v_trigger record;
begin
  for v_trigger in
    select n.nspname schema_name,c.relname table_name,t.tgname trigger_name,p.oid::regprocedure trigger_function
    ...
    where not t.tgisinternal and n.nspname='public'
      and pg_get_functiondef(p.oid) ~* 'production_workflow_command\s*\('
  loop
    raise exception 'Trigger %.% (%) can re-enter production_workflow_command',v_trigger.table_name,v_trigger.trigger_name,v_trigger.trigger_function using errcode='23514';
  end loop;
end $guard$;
...
comment on function public.production_workflow_command(text,text,timestamptz,jsonb,text,text) is 'Sole atomic Production/Order lifecycle authority. No table trigger may call or re-enter this function.';
