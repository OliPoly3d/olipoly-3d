...
assert.deepEqual(A.totalsRows(invoice).filter(r=>['Product subtotal','Sales tax','Invoice total','Total due'].includes(r[0])),[['Product subtotal',20.5],['Sales tax',1.33],['Invoice total',21.83],['Total due',21.83]]);
...
assert.deepEqual(A.totalsRows(paid).at(-1),['Total due',0]);
assert.equal(A.totalsRows(invoice).some(row => ['Deposit / prior payment','Amount paid'].includes(row[0])),false,'zero-value payment rows stay hidden');
...
assert.deepEqual(A.totalsRows(aggregate).map(r=>r[0]),['Accepted order total','Total due']);
...
assert.match(orders,/order\.payment_status === 'unpaid' \? 'Due on completion'/);
